...
func Decrypt(theBytes string) string {
	...
	if len(ciphertext) < aes.BlockSize {
		log.Println("Error Decrypt: ciphertext too short", theBytes)
	}
	iv := ciphertext[:aes.BlockSize]
...
func MakeNonce(formUid int, userName string) string {
	...
	showString := randomString
	randomString += "," + strconv.Itoa(theLen)

	theString := getNonceString(randomString)

	return theString + "|" + showString
}
...
func CheckNonce(nonce string) bool {
	...
	theLen := len(parts[1])
	newString := parts[1] + "," + strconv.Itoa(theLen)
...
func getNonceString(theString string) string {
	nonceKey := []byte(nonce)
